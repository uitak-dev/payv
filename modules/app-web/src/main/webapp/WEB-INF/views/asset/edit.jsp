...
<div class="app-shell shadow-sm">
    <header class="sticky top-0 z-30 border-b border-slate-200 bg-white">
        <div class="flex items-center justify-between px-4 py-3">
            <a href="${ctx}/asset/assets" class="rounded-lg border border-slate-200 px-3 py-1.5 text-sm">뒤로</a>
            <h1 class="text-base font-semibold">자산 수정</h1>
    ...
    <main class="px-4 pt-4 pb-24">
        ...
        <form method="post" action="${ctx}/api/asset/assets/${asset.assetId}" class="space-y-4" data-ajax="true" data-method="PUT" data-json="true">
            <section class="pv-card p-4">
                <div class="grid gap-4">
                    <label>
                        <div class="text-sm font-medium">이름</div>
                        <input name="newName" required class="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" value="${asset.name}"/>
                    </label>
                    <label>
                        ...
                        <select name="assetType" class="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
                            <option value="CASH" ${asset.assetType == 'CASH' ? 'selected' : ''}>현금</option>
                            <option value="CARD" ${asset.assetType == 'CARD' ? 'selected' : ''}>카드</option>
                            <option value="BANK_ACCOUNT" ${asset.assetType == 'BANK_ACCOUNT' ? 'selected' : ''}>계좌</option>
                        </select>
                    </label>
                </div>
            </section>
